\input{../../latex/preamble}
\input{../../latex/format}
\input{../../latex/commands}

\usepackage[style=numeric, citestyle=ieee]{biblatex}
\addbibresource{references.bib}

\begin{document}

\begin{Large}
    \textbf{EDB18802 --- Eletrônica Digital II}
    
    Atividade 4 --- Contadores assíncronos \hfill \lang{}{\DAY~de \MONTH~de \YEAR}
\end{Large}

\vspace{1ex}
\textbf{\lang{Lecturer:}{Professor:}} \text{João Cláudio Elsen Barcellos}, \href{mailto:joao.barcellos@ifsc.edu.br}{\texttt{joao.barcellos@ifsc.edu.br}}\\
\textbf{\lang{Student:}{Estudante:}}

\vspace{2ex}

%%%% PR1
\begin{problem}{Contador assíncrono de 3 bits}{pr1}

Projete um contador assíncrono, de 3 bits, utilizando qualquer tipo de FF (e.g., FF-D, FF-JK), que conte de 0 a 6. Apresente o circuito desenvolvido.

\end{problem}

%%%% PR2
\begin{problem}{Simulação}{pr2}

Monte o circuito da questão anterior no Logisim e verifique se o funcionamento corresponde ao que foi projetado. \\

\end{problem}

%%%% PR3
\begin{problem}{Montagem}{pr3}
    
Depois de resolvido o circuito anterior, ``programe-o'' no \textit{DE10-Lite Development and Education Board} do laboratório. Obs.: chame o Prof. para aprender a fazer isto. 

\end{problem}

%%%% OBS
\begin{obs}

Deve-se fazer um relatório contendo os elementos essenciais: \textit{1)} introdução com contextualização; \textit{2)} objetivos; \textit{3)} descrição dos materiais e métodos adotados; \textit{4)} resultados apresentados de forma organizada (e.g., por meio de tabelas, figuras, quando aplicável); \textit{5)} discussões com interpretações quanto aos resultados; \textit{6)} e as conclusões para sintetizar as principais descobertas e aprendizados.

\end{obs}

\newpage

\printbibliography

\end{document}
