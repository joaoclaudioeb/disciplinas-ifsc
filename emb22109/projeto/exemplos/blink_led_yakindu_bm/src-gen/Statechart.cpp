...
sc_integer Statechart::getTemp() const
{
	return temp
	;
}

void Statechart::setTemp(sc_integer temp_)
{
	this->temp = temp_;
}
...
{
	/* Entry action for state 'Off'. */
	timerService->setTimer(this, (sc_eventid)(&timeEvents[0]), ((sc_time) 100), false);
	setTemp(ifaceOperationCallback->readTemp());
}
...
{
	/* Entry action for state 'On'. */
	timerService->setTimer(this, (sc_eventid)(&timeEvents[1]), ((sc_time) 100), false);
	setTemp(ifaceOperationCallback->readTemp());
}
